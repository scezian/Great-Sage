...
_MAX_CACHE = 50            # evict oldest entry when cache exceeds this size
...
def _fetch_lyrics(artist: str, title: str) -> list | None:
    ...
    if len(_lyrics_cache) >= _MAX_CACHE:
...
def _current_line_index(lines: list, pos: float) -> int:
    ...
    pos = max(0.0, pos)  # guard against negative wall-clock drift
    idx = 0
...
class LyricsWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._destroyed    = False     # flag to guard against post-destroy races
        self._build()
    ...
    def _poll(self):
        ...
        destroyed_ref = lambda: self._destroyed  # capture for thread
        def _fetch():
            if destroyed_ref():
                return
    ...
    def _apply(self, track, lines, pos: float, status: str):
        original_lines = lines   # preserve None vs [] distinction
        if track is None:
    ...
    def cleanup(self):
        """Stop polling timer and flag widget as destroyed."""
...
def on_disable(parent, api):
    """Stop polling and threads when the plugin is disabled."""
    global _slot_widget
    if _slot_widget:
        _slot_widget.cleanup()


def cleanup(parent, api):
    """Full teardown — called when plugin is unloaded."""
    global _slot_widget
    if _slot_widget:
        _slot_widget.cleanup()
        _slot_widget.deleteLater()
        _slot_widget = None
