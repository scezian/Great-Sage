...
class AudioCapture:
    ...
    def _run(self):
        ...
        prev_lock = threading.Lock()  # protect prev array shared with audio thread
            ...
            try:
                ...
                if self._test_mode:
                    with self._lock:
                        ...
                        phase_t = self._test_time * 0.1
                        
                        # Create rhythmic pattern
                        beat_phase = (phase_t % 2.0) / 2.0  # 0 to 1 every 2 seconds
                        ...
                        for i in range(self.BAR_COUNT):
                            if i < 10:  # Bass frequencies
                                ...
                                base = 0.3 + 0.3 * math.sin(phase_t * 2)
                            else:  # Treble frequencies
                                base = 0.1 + 0.2 * math.sin(phase_t * 8)
...
class AmbientBackground(QWidget):
    ...
    def _tick(self):
        ...
        if n > 0:
            ...
            mid    = sum(bars[n//6:n//2])  / max(n//2 - n//6, 1)
            treble = sum(bars[n//2:])      / max(n - n//2, 1)
            s = 0.75
...
class AmbientControlPanel(QWidget):
    ...
    def _on_device_changed(self, idx):
        ...
        device_id = self.device_combo.currentData()
        if device_id == -1:  # "No input devices found" sentinel
            return
        if self._capture:
            debug_log(f"Switching to device {device_id}")
            self._capture.stop()
            self._capture.start(device=device_id)  # None = system default
...
def build_page(parent, api):
    ...
    if _control_widget is not None:
        try:
            _control_widget.isVisible()  # probes C++ side — throws if deleted
            if _control_widget.parent() is None:
                debug_log("build_page: discarding orphan _control_widget")
                _control_widget.deleteLater()
                _control_widget = None
        except RuntimeError:
            debug_log("build_page: discarding C++-deleted _control_widget")
            _control_widget = None
