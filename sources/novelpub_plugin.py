...
class NovelPubPlugin(SourcePlugin):
    id = "novelpub"
    name = "NovelPub"
    base_url = "https://www.novelpub.com"
    ...
    def can_handle(self, url: str) -> bool:
        return "novelpub.com" in urlparse(url).netloc
    ...
    def fetch_chapter(self, url: str, session, scraper=None) -> ChapterResult:
        ...
        # Primary: div.chapter-content (class, not id)
        content_div = soup.find("div", class_="chapter-content")
        if not content_div:
            # Some NovelPub pages use id
        ...
        # Construct prev from chapter num if nav not found
        if not prev_url and ch_num and ch_num > 1:
            prev_url = re.sub(r"/chapter-\d+", f"/chapter-{ch_num - 1}", url)
    ...
    def _nav(self, soup, is_next: bool, base: str):
        ...
        cls = re.compile(r"chapter-next" if is_next else r"chapter-prev", re.I)
    ...
    def search(self, query: str, session) -> list[SearchResult]:
        url = f"https://www.novelpub.com/search?keyword={query}&page=1"
        try:
            ...
            for item in soup.select(".novel-list .novel-item, .list-novel li"):
                ...
                desc_el = item.select_one(".synopsis, .novel-excerpt")
