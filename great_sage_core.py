...
class NotificationStore:
    ...
    Each notification is a dict with these keys:
        id        — unique string (e.g. "update-1.4.2", "rec-<uuid>")
        type      — "update" | "friend_rec"
        title     — short display string
        timestamp — ISO 8601 UTC string
        read      — bool
        data      — type-specific payload dict

    Thread-safety: all mutations are protected by a threading.Lock so
    background threads (sync poll, update checker) can call add() safely.
    """
    ...
    def _load(self):
        try:
            ...
            log.warning("NotificationStore: failed to load", error=str(e))
            self._items = []

    def _save(self):
        """Write notifications to disk. Call with _mutex held."""
        try:
            tmp = self._path + ".tmp"
            with open(tmp, "w") as f:
            ...
            log.warning("NotificationStore: failed to save", error=str(e))

    # ── Public API ────────────────────────────────────────────────────────────

    def add(self, notif_type: str, title: str, data: dict,
            notif_id: str | None = None) -> dict:
        """
        Add a new unread notification. Returns the created item.
        If a notification with the same id already exists it is not duplicated.
        """
        from datetime import timezone as _tz
        nid = notif_id or f"{notif_type}-{datetime.now(_tz.utc).timestamp():.0f}"
        with self._mutex:
            # Deduplicate by id
            existing_ids = {n["id"] for n in self._items}
            ...
            self._save()
            log.info("Notification added", id=nid, type=notif_type)
            return item

    def all_items(self) -> list[dict]:
        """Return a shallow copy of all notifications, newest first."""
        with self._mutex:
