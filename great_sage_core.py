...
class _SageCompanionWorker(QThread):
    ...
    def _fetch_web_context(self, query: str) -> str:
        """Fetch a brief web context via DuckDuckGo instant answer API (no key needed)."""
        try:
    ...
    def run(self):
        ...
        # ── Web search path ───────────────────────────────────────────────
        if self.web_search:
            context = self._fetch_web_context(q)
            if context and not context.startswith("[Web fetch failed"):
                prompt = (
                    f"You are a helpful assistant. The user asked: '{q}'\n\n"
                    f"Here is relevant web information:\n{context}\n\n"
                    f"Write a clear, concise answer based on the above. "
                    f"Do not mention that you searched the web."
                )
            else:
                prompt = (
                    f"You are a helpful assistant. The user asked: '{q}'\n\n"
                    f"Answer clearly and concisely based on your knowledge."
                )
            try:
        ...
        # ── Chapter search path (Who is / What is / general) ─────────────
