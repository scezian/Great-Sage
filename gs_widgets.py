...
def tag(text, color=MUTED):
    ...
    l.setStyleSheet(
        ...
        f"font-size:13px; letter-spacing:1px; padding:1px 6px; border-radius:2px;")
    return l
...
class NavRail(QFrame):
    ...
    _ITEMS = [
        ("dashboard", "fa5s.tachometer-alt", "DASH"),
        ("legion",    "fa5s.book-open",      "LEGION"),
        ("matrix",    "fa5s.th-large",       "MATRIX"),
        ("sage",      "fa5s.hat-wizard",     "SAGE"),
        ("editor",    "fa5s.pen",            "EDITOR"),
        ("plugins",   "fa5s.puzzle-piece",   "PLUGINS"),
    ]
    _FALLBACK_GLYPHS = {
        "dashboard": "◈", "legion": "◎", "matrix": "▣",
        "sage": "✦", "editor": "✎", "plugins": "⬡",
    }
    ...
    def _build(self):
        ...
        # ── Sliding selection indicator — glides between tiles instead of jumping ──
        from PyQt6.QtCore import QPropertyAnimation, QEasingCurve, QVariantAnimation
        from PyQt6.QtGui import QColor as _QColor
        self._pill = pill
        self._indicator = QWidget(pill)
        self._indicator.setObjectName("NavIndicator")
        self._indicator.setFixedSize(3, 34)
        self._indicator.setStyleSheet(f"background:{ACCENT}; border-radius:2px;")
        self._indicator.hide()
        self._indicator_anim = QPropertyAnimation(self._indicator, b"pos")
        self._indicator_anim.setDuration(260)
        self._indicator_anim.setEasingCurve(QEasingCurve.Type.OutCubic)

        # ── Ambient breathing border on the rail pill itself ───────────────────────
        self._border_anim = QVariantAnimation(self)
        self._border_anim.setStartValue(_QColor("#16161E"))
        self._border_anim.setKeyValueAt(0.5, _QColor(f"{ACCENT}30"))
        self._border_anim.setEndValue(_QColor("#16161E"))
        self._border_anim.setDuration(4200)
        self._border_anim.setLoopCount(-1)
        self._border_anim.valueChanged.connect(self._paint_pill_border)
        ...
        self._rail_menu_btn.setStyleSheet(f"""
            QPushButton {{
                ...
                color:#6A6A88; font-size:18px; padding:0;
            }}
        ...
        self._net_dot.setStyleSheet(
            f"color:{ACCENT2}66; font-size:13px; background:transparent; border:none;")
    ...
    def _set_icon(self, lbl, icon_name: str, color: str, size: int = 17):
        """Render icon_name at the given color into lbl. Falls back to
        the original Unicode glyph as plain text if qtawesome isnt installed."""
        if _HAS_QTA:
            lbl.setPixmap(qta.icon(icon_name, color=color).pixmap(size, size))
            lbl.setProperty("icon_name", icon_name)
        else:
            lbl.setText(self._FALLBACK_GLYPHS.get(icon_name, "•"))
            lbl.setStyleSheet(f"background:transparent; border:none; font-size:{size}px; color:{color};")
    def _make_tile(self, key: str, icon: str, label: str) -> QWidget:
        tile = QWidget()
        tile.setFixedSize(52, 58)
        ...
        self._set_icon(ico, icon, "#6A6A88")
        ...
        txt.setStyleSheet(
            ...
            f"font-family:{FONT_UI}; font-size:13px; letter-spacing:1px; color:#5A5A78;")
    ...
    def _apply_style(self, tile: QWidget, active: bool):
        if active:
            tile.setStyleSheet(
                "QWidget { background:#1A1106; border:1px solid transparent; border-radius:10px; }")
            self._set_icon(tile._ico, tile._ico.property("icon_name"), ACCENT)
            tile._txt.setStyleSheet(
                f"background:transparent; border:none; "
                f"font-family:{FONT_UI}; font-size:13px; letter-spacing:1px; "
            ...
            self._set_icon(tile._ico, tile._ico.property("icon_name"), "#6A6A88")
            tile._txt.setStyleSheet(
                f"background:transparent; border:none; "
                f"font-family:{FONT_UI}; font-size:13px; letter-spacing:1px; color:#5A5A78;")

    def _pulse_tile(self, tile: QWidget):
        """Subtle heartbeat glow pulse on press."""
        ...
        anim = QPropertyAnimation(effect, b"blurRadius", tile)
        anim.setDuration(360)
        anim.setKeyValueAt(0.0, 0)
        anim.setKeyValueAt(0.3, 20)
    ...
    def set_active(self, key: str):
        ...
        if key in self._btns:
            ...
            self._glide_indicator_to(key)

    def _glide_indicator_to(self, key: str):
        from PyQt6.QtCore import QPoint
        tile = self._btns.get(key)
        if not tile:
            return
        target = QPoint(6, tile.y() + (tile.height() - self._indicator.height()) // 2)
        if self._indicator.isVisible():
            self._indicator_anim.stop()
            self._indicator_anim.setStartValue(self._indicator.pos())
    ...
    def _paint_pill_border(self, color):
...
class ReadingRoomOverlay(QWidget):
    ...
    def _make_top_bar(self):
        ...
        genre_lbl.setStyleSheet(
            "color: rgba(255,255,255,0.55); font-size:13px; letter-spacing:3px;")

        snd_lbl = QLabel("♪")
        snd_lbl.setStyleSheet("color:rgba(255,255,255,0.55); font-size:16px;")
    ...
    def _make_bot_bar(self, title):
        ...
        h.setContentsMargins(28, 0, 28, 0)
        tl = QLabel(title[:80])
        tl.setStyleSheet("color:rgba(255,255,255,0.50); font-size:12px; letter-spacing:1px;")
        el = QLabel("Esc · exit   |   Space · scroll")
        el.setStyleSheet("color:rgba(255,255,255,0.30); font-size:13px; letter-spacing:1px;")
  ...
  s += SHELF_SIZE) {
    ...
    batch.forEach((book, i) => {
      ...
      el.innerHTML = `
        ...
        <div class="book-tooltip">${esc(book.title)}<br><span style="color:#4A6080;font-size:12px">${esc(book.info)}</span></div>
      `;
...
class MemoryPalaceWindow(QWidget):
    ...
    def _build(self):
        ...
        tl.setStyleSheet(f"color:{ACCENT};font-size:13px;letter-spacing:3px;")
    ...
    def _build_native(self, root):
        ...
        lib_hdr.setStyleSheet(f"color:#1E3050;font-size:13px;letter-spacing:3px;")
            ...
            for shelf_start in range(0, len(books), SHELF_SIZE):
                ...
                for i, (title, ch_read) in enumerate(batch):
                    ...
                    spine.setStyleSheet(
                        ...
                        f"font-size:12px;padding:2px;border-radius:1px 0 0 1px;")
        ...
        view_hdr.setStyleSheet(f"color:#1E3050;font-size:13px;letter-spacing:3px;")
        ...
        if not shows:
            ...
            for i, (title, eps) in enumerate(shows):
                ...
                ico_lbl.setStyleSheet("font-size:25px;background:transparent;")
                t_lbl = QLabel(title[:20])
                t_lbl.setAlignment(Qt.AlignmentFlag.AlignBottom | Qt.AlignmentFlag.AlignHCenter)
                t_lbl.setWordWrap(True)
                t_lbl.setStyleSheet("font-size:12px;color:rgba(255,255,255,0.75);background:transparent;")
...
class WatchfaceWindow(QWidget):
    ...
    def _build(self):
        ...
        self._clock_lbl.setStyleSheet(
            f"font-family:{FONT_DISPLAY}; font-size:110px; font-weight:bold;"
            f"color:{TEXT}; letter-spacing:4px;")
        self._clock_lbl.setAlignment(Qt.AlignmentFlag.AlignLeft)
        root.addWidget(self._clock_lbl)

        self._date_lbl = QLabel()
        self._date_lbl.setStyleSheet(f"color:{MUTED}; font-size:21px; letter-spacing:3px;")
        ...
        row1 = QHBoxLayout()
        row1.setSpacing(32)
        ico1 = QLabel("◈"); ico1.setStyleSheet(f"color:{ACCENT}; font-size:41px;")
        col1 = QVBoxLayout()
        col1.setSpacing(4)
        self._reading_title = QLabel("—")
        self._reading_title.setStyleSheet(
            f"font-family:{FONT_DISPLAY}; font-size:25px; color:{TEXT}; letter-spacing:1px;")
        self._reading_sub = QLabel("READING")
        self._reading_sub.setStyleSheet(f"color:{MUTED}; font-size:12px; letter-spacing:3px;")
        ...
        row2 = QHBoxLayout()
        row2.setSpacing(32)
        ico2 = QLabel("◎"); ico2.setStyleSheet(f"color:{BLUE}; font-size:41px;")
        col2 = QVBoxLayout()
        col2.setSpacing(4)
        ...
        self._watching_sub.setStyleSheet(f"color:{MUTED}; font-size:12px; letter-spacing:3px;")
        ...
        row3 = QHBoxLayout()
        row3.setSpacing(32)
        ico3 = QLabel("✦"); ico3.setStyleSheet(f"color:{PURPLE}; font-size:41px;")
        col3 = QVBoxLayout()
        col3.setSpacing(4)
        ...
        self._sage_sub.setStyleSheet(f"color:{MUTED}; font-size:12px; letter-spacing:3px;")
        ...
        hint.setStyleSheet(f"color:{MUTED}; font-size:13px; letter-spacing:2px;")
    ...
    def _apply_style(self, active: bool):
        if active:
            self.setStyleSheet(
                ...
                f"border-radius:3px;color:{ACCENT};font-size:12px;letter-spacing:1px;"
                f"padding:8px 14px;}}"
...
class NotificationPanel(QFrame):
    ...
    def _build(self):
        ...
        self._mark_all_btn.setStyleSheet(
            ...
            f"color:{MUTED};font-size:13px;letter-spacing:1px;padding:2px 6px;}}"
    ...
    def reload(self):
        ...
        if not items:
            ...
            empty.setStyleSheet(
                f"color:{MUTED};font-size:13px;padding:32px 16px;"
        ...
        if ntype == "update":
            ...
            if version:
                ...
                ver_lbl.setStyleSheet(
                    f"color:{ACCENT};font-size:13px;letter-spacing:1px;"
                    f"background:transparent;border:none;")
    ...
    def _show_update_detail(self, data: dict):
        ...
        if commits:
            for c in commits:
                ...
                sha_lbl.setStyleSheet(
                    f"color:{MUTED};font-size:13px;font-family:monospace;")
                msg_lbl = QLabel(msg)
        ...
        ok_btn.setStyleSheet(
            ...
            f"color:{TEXT2};border-radius:6px;padding:6px 20px;font-size:13px;")
...
# ═══════════════════════════════════════════════════════════════════════════════
# GLOW TAB BAR — rounded, glowing, hover-lift tabs with a sliding indicator
# ═══════════════════════════════════════════════════════════════════════════════
from PyQt6.QtWidgets import QTabBar
from PyQt6.QtCore import Qt, QRectF, QSize, QVariantAnimation, QEasingCurve
from PyQt6.QtGui import QPainter, QColor, QPen, QBrush, QFont, QFontMetrics


class GlowTabBar(QTabBar):
    """
    Custom-painted QTabBar:
      - rounded pill background behind hovered/selected tabs
      - glowing ring around the selected tab (simulated bloom, no live blur)
      - tabs lift a few px on hover
      - short 'heartbeat' pulse on click
      - bottom indicator bar animates (slides) between tab positions
        instead of snapping via QSS border-bottom
    """

    def __init__(self, accent=None, muted=None, text=None, text2=None,
                 bg2=None, bg3=None, parent=None):
        super().__init__(parent)
        self._accent = accent or ACCENT
        self._muted  = muted  or MUTED
        self._text   = text   or TEXT
        self._text2  = text2  or TEXT2
        self._bg2    = bg2    or BG2
        self._bg3    = bg3    or BG3
        ...
        self._ind_x = 0.0
        self._ind_w = 0.0
        self._ind_start = (0.0, 0.0)
        self._ind_end = (0.0, 0.0)
        self._ind_anim = QVariantAnimation(self)
        self._ind_anim.setDuration(280)
        self._ind_anim.setStartValue(0.0)
        self._ind_anim.setEndValue(1.0)
        self._ind_anim.setEasingCurve(QEasingCurve.Type.OutCubic)
        self._ind_anim.valueChanged.connect(self._on_ind_step)
    ...
    def tabSizeHint(self, index):
        ...
        w = fm.horizontalAdvance(text) + int(len(text) * 2.5)
        return QSize(w + 56, 42)

    def minimumTabSizeHint(self, index):
        return self.tabSizeHint(index)

    # ---- indicator ----
    def _slide_indicator(self, index):
        if index < 0 or index >= self.count():
            return
        r = self.tabRect(index)
        target_w = max(24, int(r.width() * 0.5))
        target_x = r.x() + (r.width() - target_w) / 2
        if self._ind_w == 0.0:
            self._ind_x, self._ind_w = float(target_x), float(target_w)
            self.update()
            return
        self._ind_anim.stop()
        self._ind_start = (self._ind_x, self._ind_w)
        self._ind_end = (float(target_x), float(target_w))
        self._ind_anim.start()

    def _on_ind_step(self, progress):
        sx, sw = self._ind_start
        ex, ew = self._ind_end
        self._ind_x = sx + (ex - sx) * progress
        self._ind_w = sw + (ew - sw) * progress
        self.update()

    def showEvent(self, e):
        super().showEvent(e)
        self._slide_indicator(self.currentIndex())
    ...
    # ---- hover lift ----
    def _animate_lift(self, index, target):
        anim = self._lift_anims.get(index)
        if anim:
            anim.stop()
        anim = QVariantAnimation(self)
        anim.setDuration(150)
        anim.setEasingCurve(QEasingCurve.Type.OutCubic)
        anim.setStartValue(self._lift.get(index, 0.0))
    ...
    def mouseMoveEvent(self, e):
        ...
        if idx != self._hover_index:
            ...
            if old != -1:
                self._animate_lift(old, 0.0)
            if idx != -1:
                self._animate_lift(idx, 1.0)
        super().mouseMoveEvent(e)

    def leaveEvent(self, e):
        if self._hover_index != -1:
            self._animate_lift(self._hover_index, 0.0)
            self._hover_index = -1
        super().leaveEvent(e)

    # ---- click heartbeat ----
    def _on_clicked(self, index):
        anim = self._pulse_anims.get(index)
        if anim:
            anim.stop()
        anim = QVariantAnimation(self)
    ...
    def _paint_tab(self, painter, index):
        ...
        lift = self._lift.get(index, 0.0)
        pulse = self._pulse.get(index, 0.0)
        selected = (index == self.currentIndex())

        draw_rect = QRectF(rect).adjusted(6, 4, -6, -4)
        draw_rect.translate(0, -4.0 * lift)

        if selected or lift > 0 or pulse > 0:
            bg = QColor(self._bg3)
            bg.setAlphaF(min(1.0, 0.55 + 0.25 * max(lift, pulse)))
            painter.setPen(Qt.PenStyle.NoPen)
        ...
        painter.drawText(draw_rect, Qt.AlignmentFlag.AlignCenter, self.tabText(index))
